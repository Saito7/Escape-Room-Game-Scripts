...
public class DarkCabinetGlassDoorInteract2 : MonoBehaviour, IInteractable,ISaveable
{
    ...
    public void Interact(GameObject other)
    {
        if (doorAnimator != null)
        {
            if (!doorOpen)
            {
                doorAnimator.Play("GlassDoorOpening2");
            ...
            {
                doorAnimator.Play("GlassDoorClosing2");
    ...
    public void RestoreState(object state)
    {
        ...
        if (doorOpen)
        {
            GetComponent<Animator>().Play("GlassDoorOpening2");
            interactText = "close the";
        }
    }
}
