...
public class DungeonEntranceDoorInteract : MonoBehaviour, IInteractable,ISaveable
{
    ...
    public void Interact(GameObject other)
    {
        if (doorAnimator != null)
        {
            if (!doorOpen)
            {
                doorAnimator.Play("DungeonEntranceDoorOpening");
                doorOpening.Play();
                doorOpen = true;
            ...
            {
                doorAnimator.Play("DungeonEntranceDoorClosing");
                doorClosing.Play();
                doorOpen = false;
    ...
    public void RestoreState(object state)
    {
        ...
        if (doorOpen)
        {
            GetComponent<Animator>().Play("DungeonEntranceDoorOpening");
            interactText = "close the";
        }
    }
}
