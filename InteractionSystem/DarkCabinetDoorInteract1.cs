...
public class DarkCabinetDoorInteract1 : MonoBehaviour, IInteractable,ISaveable
{
    ...
    public void Interact(GameObject other)
    {
        if (doorAnimator != null)
        {
            if (!doorOpen)
            {
                doorAnimator.Play("CabinetDoorOpening1");
            ...
            {
                doorAnimator.Play("CabinetDoorClosing1");
    ...
    public void RestoreState(object state)
    {
        ...
        if (doorOpen)
        {
            GetComponent<Animator>().Play("CabinetDoorOpening1");
            interactText = "close the";
        }
    }
}
