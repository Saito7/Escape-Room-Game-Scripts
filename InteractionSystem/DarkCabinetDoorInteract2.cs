...
public class DarkCabinetDoorInteract2 : MonoBehaviour, IInteractable,ISaveable
{
    ...
    public void Interact(GameObject other)
    {
        if (doorAnimator != null)
        {
            if (!doorOpen)
            {
                doorAnimator.Play("CabinetDoorOpening2");
            ...
            {
                doorAnimator.Play("CabinetDoorClosing2");
    ...
    public void RestoreState(object state)
    {
        ...
        if (doorOpen)
        {
            GetComponent<Animator>().Play("CabinetDoorOpening2");
            interactText = "close the";
        }
    }
}
